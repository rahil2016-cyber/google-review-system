...
function resolveRole(email: string): string {
  if (email.includes("owner") || email.includes("admin")) return "BUSINESS_ADMIN";
  return "STAFF";
}

export async function ensureUserForLogin(input: { tenantId: string; email: string }): Promise<{
  userId: string;
  role: string;
}> {
...
export async function issueSessionTokens(input: {
  ...
  const accessToken = jwt.sign(
    { sub: input.subject, tenantId: input.tenantId, role: input.role, uid: input.userId },
    env.JWT_SECRET,
